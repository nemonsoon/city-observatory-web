...
厳密な AQI は採用せず、PM2.5 の値だけで4段階に分ける。利用者が一目で判断できることを優先した。

境界値には環境省の公的な数値を使う。対象地域が日本の主要6都市なので、読み手が判断に使う基準と表示の基準を揃えた。

| PM2.5（μg/m³） | 画面の表示 | 段階 | 境界の根拠                     |
| -------------- | ---------- | ---- | ------------------------------ |
| 0 – 15         | 良好       | 1    | 環境基準の年平均値             |
| 15.1 – 35      | 普通       | 3    | 環境基準の日平均値             |
| 35.1 – 70      | 悪い       | 4    | 注意喚起の暫定指針値の手前     |
| 70 超          | 危険       | 5    | 注意喚起の暫定指針値を超える値 |

日本には PM2.5 を多段階に区分する全国共通の公的な制度がない。全国統一で定められているのは次の2つで、この4段階は環境省の区分ではなく、公的な数値3つを段階の境界に割り当てたもの。

| 種別                                 | 数値                                           | 出典                                                                             |
| ------------------------------------ | ---------------------------------------------- | -------------------------------------------------------------------------------- |
| 環境基準                             | 年平均値 15 μg/m³ 以下、日平均値 35 μg/m³ 以下 | 微小粒子状物質による大気の汚染に係る環境基準について（平成21年環境省告示第33号） |
| 注意喚起のための暫定的な指針となる値 | 日平均値 70 μg/m³ 超                           | 環境省 微小粒子状物質に関する専門家会合（2013年2月27日）                         |

- 環境省 微小粒子状物質（PM2.5）に関する情報: https://www.env.go.jp/air/osen/pm/info.html
- 環境基準の告示本文: https://www.env.go.jp/content/900479267.pdf
- 注意喚起の暫定指針を示した専門家会合報告: https://www.env.go.jp/content/900403743.pdf

段階は [デザインシステム](../DESIGN.md) の Level Scale に対応する。実装は `lib/domain/air-quality-label.ts` と `lib/domain/observation-level.ts`。快適度スコア（`lib/domain/comfort-score.ts`）も年平均値の 15 を良好域として使う。
